...
export default function FutureDemands() {
  ...
  useEffect(() => {
    if (selectedCategory) {
      fetch(`http://127.0.0.1:5003/future-demand/${selectedCategory}`)
  ...
  const handleDrugClick = (drug: string) => {
    navigate('/drug-demand-chart', { state: { drug, category: selectedCategory } });
  };
            ...
            <table className="min-w-full bg-zinc-800 border border-zinc-700 rounded">
              ...
              <tbody className="divide-y divide-zinc-600">
                {drugs.map((drug, index) => (
                  <tr key={index} onClick={() => handleDrugClick(drug)} className="cursor-pointer hover:bg-zinc-700 transition-colors">
