...
main_folder = r'D:\Cardiovascular'
medicine_categories = ['AMLODIP', 'ASPIRIN', 'CLOPIDOGREL', 'ENALA', 'LOSARTAN']
...
def extract_dosage(item_name):
    ...
    match = re.search(r'(\d+(\.\d+)?MG)', str(item_name))  # This captures both integers and decimals followed by MG
    if match:
...
output_path = r'D:\Structured_CardiovascularFINAL_Data.xlsx'
combined_data.to_excel(output_path, index=False, sheet_name='Cardiovascular')
